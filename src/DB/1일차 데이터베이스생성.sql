...
- 관계형 데이터베이스 RDBMS
    ...
    - 행과 열로 이루어진 각각의 데이블을 !!(고유값)을 참조하여 서로 종속되는 관계(=연결)를 표현
    
- 테이블 기본용어
	- 레코드/튜플/행/가로 : 관계된 데이터들의 가로 묶음
    - 필드/속성/열/세로 : 가장 작은 다위 데이터
...
# (워크벤치) 상단메뉴에서 4번째 아이콘 			: create new schema 클릭
...
# 5. 사용할 데이터베이스 활성화/선택				: use 데이터베이스이름
	# 데이터베이스 안에 여러개 테이블을 저장할수 있는데 테이블 조작하기 전에 먼저 데이터베이스 선택
use sqldb;
