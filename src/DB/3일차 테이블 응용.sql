...
create table member(				# 'member' 테이블 생성	[create table 테이블명(필드명 타입,필드명 타입,필드명 타입)]
    ...
    mphone char(13),	-- 회원전화번호(고정길이)
    mname varchar(10),	-- 회원명(최대 10글자)
    confirm boolean,	-- 광고문자전송여부 true/false
...
insert into member2 values(1,2);			-- (전체 필드에 값 대입시)회원번호1 필드에 1 대입 , 회원번호2 필드에 2 대입
insert into member2 (회원번호1) values(1);		-- (특정 필드에 값 대입시 필드명 명시/작성)			오류 : 회원번호2 필드가 not null이기 때문에
insert into member2 (회원번호2) values(2);		-- 회원번호1 필드에 null자동대입 , 회원번호2 필드에 2대입
select * from member2;
...
create table member3(
	회원번호1 int,								
    회원번호2 int default 0,					-- default 값/함수
...
insert into member3 values(10,20,'일본',false,'2024-01-22 16:44:00');				-- 레코드 삽입시 필드값이 있을경우 대입
select * from member3;

# 3. unique : 중복된 값을 저장할 수 없다
drop table if exists member4;
create table member4(
	회원번호1 int,
	회원번호2 int unique						-- unique : 필드내 중복된 값을 제거
);
...
# 5. foreign key(외래키)
	# 1. 다른 테이블의 기본키 정보와 연결/참조된 정보 (= 참조)

# ------ 회원제 게시판 : (회원들이 글을 작성해서 저장)------ #
# 계층구조 : 회원(상위) 게시판(하위)
# 테이블 1개 : 서로다른 목적의 데이터들을 하나의 테이블로 작성시 이상현상(중복/불필요한정보)발생
# 테이블 2개 : 동일한 목적의 데이터끼리 테이블을 생성해서 연결/관계(pk-fk) 이용한 관계형 데이터베이스 구축

# 5-1 회원테이블
	# 1.회원번호 / 2.아이디 / 3.비밀번호 / 4.이름
    # 필드별 타입 선정 -> 제약조건 -> pk선정(식별키 : 중복X , 공백X , 하나의 레코드의 대표)
drop table if exists member5;
create table member5(
	mno int not null unique,				-- not null이면서 중복도 없는 필드
    mid varchar(30) not null unique,		-- not null
    mpw varchar(30) not null,				-- not null
    mname varchar(20) default '익명',			-- 기본값 이름이 '익명' 하지만 insert하면 해당값으로 저장
    primary key(mno)						-- mno 필드를 기본키(pk)로 사용 / 관례적으로 테이블 1개당 pk 1개이상
);
...
create table board(
    ...
    foreign key(mno_fk) references member5(mno)		# foreign key(fk로사용할필드) references pk필드테이블명(pk필드)
);