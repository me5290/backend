...
# 3. 테이블 생성
create table board(
	bno bigint,				-- 게시물번호 (게시물 번호 20억 번호 이상)
    btitle varchar(100), 	-- 게시물제목 (게시물 제목을 100글자 이내 가변길이)
    bcontent longtext,		-- 게시물내용 (텍스트만 지원하는 내용 - 사진,동영상 최대 4G 지원)
    bview int, 				-- 게시물조회수
    bdate date				-- 게시물작성일
);
